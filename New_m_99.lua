...
-- Create Tabs
local MainTab = CreateTab("Bring")

-- BRING ALL (FIXED - ONLY REAL ITEMS)
...
local function BringAllItems()
    ...
    -- STRICT WHITELIST (ТОЛЬКО ИЗ ВИКИ)
    local whitelist = {
        -- Basic Resources
        "log",
        "stick",
    ...
    -- BLACKLIST (ГОВНО КОТОРОЕ ТОЧНО НЕ НАДО)
    local blacklist = {
        -- Постройки
        "campfire", "bonfire", "fire", "fireplace",
        "tent", "shelter", "wall", "floor",
        
        -- Природа
        "tree", "bush", "grass", "stone", "rock",
        "rainbow", "cloud", "sun", "moon",
        
        -- Животные
        "deer", "wolf", "bear", "rabbit", "mammoth",
        "animal", "creature", "beast",
        
        -- Декор
        "plank", "board", "decoration", "prop"
    }
    ...
    local function isWhitelisted(name)
        ...
        for _, item in pairs(whitelist) do
            if lower == item or lower:find("^" .. item) then
                return true
            end
        end
        return false
    ...
    for _, obj in pairs(game.Workspace:GetDescendants()) do
        pcall(function()
            ...
            -- THEN WHITELIST
            if not isWhitelisted(obj.Name) then return end
            ...
            if obj:IsA("Model") then
                ...
                if handle and handle:IsA("BasePart") then
                    -- Only small items (not buildings)
                    if handle.Size.Magnitude < 10 then
        ...
        -- Parts (small only)
            if (obj:IsA("BasePart") or obj:IsA("MeshPart")) and obj.Size.Magnitude < 8 then
                table.insert(items, obj)
            end
        end)
    end
    
    -- Teleport in CIRCLE with radius 15
    local radius = 15
    local angleStep = (2 * math.pi) / math.max(#items, 1)
    ...
    for i, item in ipairs(items) do
        pcall(function()
            ...
            item.CFrame = hrp.CFrame * CFrame.new(offsetX, 0, offsetZ)
        end)
    end
    
    if #items > 0 then
        print("✅ Brought " .. #items .. " items!")
    else
...
-- BUTTONS
CreateButton(MainTab, "🎁 BRING ALL", function()
    BringAllItems()
end)

CreateToggle(MainTab, "🔄 Auto Bring", function(state)
    BringAllActive = state
    spawn(function()
